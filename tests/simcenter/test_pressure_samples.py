...
def model(monkeypatch):
    cae = NS(Result=NS(Component=NS(Scalar=3), CoordinateSystem=NS(AbsoluteRectangular=5)))
                ...
                GetIterations=lambda: [
                    NS(
                        GetResultTypes=lambda: [
                            NS(Name="Pressure - Element-Nodal"),
                            NS(Name="Total Pressure - Element-Nodal"),
                        ]
                    )
                ]
            )
    ...
    monkeypatch.setattr(
        "nx_mcp.simcenter.pressure_samples.acquire_result", lambda *a: (result, False)
    )
    return (
...
def test_component_units_connectivity_and_undefined_elements(model):
    ...
    r = pressure_samples(session, sim, element_labels=[202, 101], field="total_pressure")
    assert [e["element_label"] for e in r["items"]] == [202, 101]
    assert r["items"][0]["defined"] is False
    assert all(n["pressure"] is None for n in r["items"][0]["nodes"])
    assert [n["node_label"] for n in r["items"][1]["nodes"]] == [1001, 1003, 1002, 1004]
    assert [n["pressure"] for n in r["items"][1]["nodes"]] == [3, 9, 6, 12]
    assert access.AskElementNodalResult.call_count == 4
    assert r["units"] == "Pa" and r["field"] == "Total Pressure - Element-Nodal"
    assert params.GetUnit() == "PressurePascals"
...
def test_invalid_labels_rejected_before_native_access(monkeypatch, labels):
    ...
    monkeypatch.setattr("nx_mcp.simcenter.pressure_samples.acquire_result", acquire)
    with pytest.raises(NXToolError):
        pressure_samples(None, None, element_labels=labels)
...
def test_invalid_native_data_rejected_with_owned_result_cleanup(model, monkeypatch, bad):
    session, sim, result, access, params, freed = model
    monkeypatch.setattr(
        "nx_mcp.simcenter.pressure_samples.acquire_result", lambda *a: (result, True)
    )
    ...
    if bad == "component":
        params.GetResultComponent = lambda: -1
    with pytest.raises(NXToolError):
        pressure_samples(session, sim, element_labels=[101])
...
def test_adapter_discards_changed_result_pages(monkeypatch, change):
    ...
    monkeypatch.setattr("nx_mcp.simcenter.pressure_samples.pressure_samples", reader)
    host = NS(
    ...
    if change == "none":
        r = SimcenterMixin._sim_pressure_samples(host, "sim", "a" * 64, [101], "total_pressure")
        assert r["result_file"] == row
        assert reader.call_args.kwargs["element_labels"] == [101]
    else:
        with pytest.raises(NXToolError, match="Result"):
            SimcenterMixin._sim_pressure_samples(host, "sim", "a" * 64, [101])
...
async def test_public_tool_is_readonly_and_requires_bound_result(tmp_path, monkeypatch):
    ...
    tool = next(t for t in await server.list_tools() if t.name == "nx_sim_pressure_samples")
...
def test_selects_requested_native_field(model, field, native):
    session, sim, result, access, params, freed = model
    pressure_samples(session, sim, element_labels=[101], field=field)
    assert params.SetGenericResultType.call_args.args[0].Name == native


@pytest.mark.parametrize("field", ["velocity", "", None])
def test_bad_field_rejected_before_acquisition(monkeypatch, field):
    acquire = Mock(side_effect=AssertionError("must not acquire"))
    monkeypatch.setattr("nx_mcp.simcenter.pressure_samples.acquire_result", acquire)
    with pytest.raises(NXToolError):
        pressure_samples(None, None, element_labels=[101], field=field)
    acquire.assert_not_called()
