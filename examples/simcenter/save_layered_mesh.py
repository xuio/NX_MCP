def run(executor):
    ...
    record = root / "fluid-layered-01.json"
    if record.exists():
    ...
    backup = root / "before-fluid-layered-01"
    ...
    try:
        ...
        if len(walls) != 4:
            raise ValueError("Expected four duct walls")
        layer = create_boundary_layers(s, fem, walls, first_layer_mm=0.1, layers=8, growth_rate=1.2)
        layer.pop("control")
