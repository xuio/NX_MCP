...
def run(executor):
    ...
    if "E-manager-export-20260908-r1" not in sim.FullPath:
        raise ValueError("Activate the retained coupled development SIM")
    sid = executor._reference(sim, "part", sim, "SIM")["id"]
    root = executor.workspace.resolve("ui-benchmarks/E-documented-controls-20260908-r1")
    if root.exists():
        raise ValueError("Inspect retained export directory; do not repeat")
    copied = executor._sim_save_as(sid, str(root / "coupled_documented_controls_r1.sim"))
    import NXOpen as nx
    ...
    key = "Coupled Solution Parameters"
    descriptor = "Thermal-Flow Coupled Solution Parameters"
    assert solution.PropertyTable.GetNamedPropertyTablePropertyValue(key) is None
    mark = executor.session.SetUndoMark(
        nx.Session.MarkVisibility.Visible, "MCP documented coupled controls"
    )
    try:
        controls = sim.ModelingObjectPropertyTables.CreateModelingObjectPropertyTable(
            descriptor,
            "NX MULTIPHYSICS - Coupled Thermal-Flow",
            "NX MULTIPHYSICS",
            "Documented coupled controls",
            0,
        )
        solution.PropertyTable.SetNamedPropertyTablePropertyValue(key, controls)
        assert solution.PropertyTable.GetNamedPropertyTablePropertyValue(key) == controls
        assert controls.DescriptorType == descriptor
        readback = read_properties(controls.PropertyTable, nx)
    except Exception:
    ...
    for path in root.iterdir():
        ...
        if path.is_file() and path.stat().st_size < 8 * 1024 * 1024:
            if path.suffix.lower() == ".xml":
                ...
                shutil.copyfile(
                    path,
                    r"Z:\nx-mcp-integration\simcenter-discovery\coupled-documented-controls-deck.xml",
                )
    ...
    return {
        "copy": copied,
        "controls_descriptor": descriptor,
        "controls_properties": readback,
