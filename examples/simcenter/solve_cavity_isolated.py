...
def run(executor):
    ...
    if type(sim).__name__ != "SimPart" or "F-input-export-20260908-r2" not in sim.FullPath:
        raise ValueError("Cavity SIM required")
    root = Path(sim.FullPath).parent
    deck = root / "flow_input_r2-Flow_benchmark.xml"
    raw = deck.read_bytes()
    ...
    def native_launch():
        ...
        preserve_input(
            executor.workspace.ensure_inside(store.root / "isolated-flow-01"),
            "before-launch",
            raw,
        )
    ...
    return {
        "job": launch_isolated(
            store, "isolated-flow-01", manifest, native_launch, output_directory=str(root)
        ),
