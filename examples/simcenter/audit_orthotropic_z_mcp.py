...
async def main():
    ...
    async with (
        ...
        output = Path(r"Z:\nx-mcp-integration\simcenter-discovery\stdio-orthotropic-z-finish.json")
        ...
        while True:
            status = await client.call_tool(
                "nx_sim_job_status",
                {"job_id": "orthotropic-z-thermal-01", "include_evidence": True},
            )
        ...
        release = await client.call_tool(
            "nx_sim_release_job",
            {"job_id": "orthotropic-z-thermal-01", "operation_id": "orthotropic-z-release-01"},
        )
        ...
        document = next(
            ...
            if d["work"] and d["path"].endswith("orthotropic_z_solve_r1.sim")
        )
