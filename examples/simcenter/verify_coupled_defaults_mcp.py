...
async def main():
    ...
    async with (
        ...
        assert "attach_defaults" in schema["properties"]["action"]["enum"]
        docs = await client.call_tool("nx_sim_documents", {"limit": 100})
        document = next(
            d["document"]["id"]
            for d in docs.structuredContent["documents"]
            if d["work"] and "E-defaults-public-20260908-r1" in d["path"]
        )
        args = {
            "document": document,
            "action": "attach_defaults",
            "operation_id": "coupled-defaults-public-r1",
        }
        ...
        saved = await client.call_tool(
            "nx_sim_save", {"document": document, "operation_id": "coupled-defaults-save-r1"}
        )
        assert not saved.isError, saved.structuredContent
        Path(r"Z:\nx-mcp-integration\simcenter-discovery\coupled-defaults-public.json").write_text(
            json.dumps(
                {
                    "schema": schema,
