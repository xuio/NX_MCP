...
async def main():
    ...
    async with (
        ...
        source = next(
            ...
            for d in docs
            if d["work"]
            and d["path"].endswith("D-independent-clone-export-20260908-r1\\independent_export.sim")
        )
        responses = {}
        output = Path(
            r"Z:\nx-mcp-integration\simcenter-discovery\stdio-independent-refined-launch.json"
        )
        ...
        copied = await client.call_tool(
            "nx_sim_save_as",
            {
                ...
                "path": "ui-benchmarks/D-independent-refined-solve-20260908-r1/refined_flow_r1.sim",
                "operation_id": "independent-refined-copy-01",
            },
        )
        ...
        prepared = await client.call_tool(
            "nx_sim_prepare_solve", {**args, "operation_id": "independent-refined-prepare-01"}
        )
        ...
        launched = await client.call_tool(
            "nx_sim_launch", {**args, "operation_id": "independent-refined-launch-01"}
        )
