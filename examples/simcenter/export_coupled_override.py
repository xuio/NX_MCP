"""Test explicit BasePropertyTable override flag from installed NXOpen.xml; export only."""
...
def run(executor):
    ...
    if "E-environment-cae-scalar-20260908-r1" not in sim.FullPath:
        raise ValueError("Activate the retained coupled development SIM")
    sid = executor._reference(sim, "part", sim, "SIM")["id"]
    root = executor.workspace.resolve("ui-benchmarks/E-environment-override-20260908-r1")
    if root.exists():
        raise ValueError("Inspect retained export directory; do not repeat")
    copied = executor._sim_save_as(sid, str(root / "coupled_override_r1.sim"))
    import NXOpen as nx
    ...
    try:
        for key, value, unit in [
            ...
            table.SetScalarWithDataPropertyValue(key, value, sim.UnitCollection.FindObject(unit))
            table.SetTablePropertyOverride(key)
    ...
    for path in root.iterdir():
        ...
        if path.is_file() and path.stat().st_size < 8 * 1024 * 1024:
            if path.suffix.lower() == ".xml":
                ...
                shutil.copyfile(
                    path, r"Z:\nx-mcp-integration\simcenter-discovery\coupled-override-deck.xml"
                )
