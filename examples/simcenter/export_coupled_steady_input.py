...
def run(executor):
    ...
    if "E-development-export-20260908-r1" not in sim.FullPath:
        raise ValueError("Activate the retained coupled development SIM")
    sid = executor._reference(sim, "part", sim, "SIM")["id"]
    root = executor.workspace.resolve("ui-benchmarks/E-development-steady-20260908-r1")
    if root.exists():
        raise ValueError("Inspect retained export directory; do not repeat")
    copied = executor._sim_save_as(sid, str(root / "coupled_steady_r1.sim"))
    import NXOpen as nx
