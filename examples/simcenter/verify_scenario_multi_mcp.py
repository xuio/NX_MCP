...
async def main():
    ...
    async with (
        ...
        output = stage / "stdio-scenario-multi.json"
        results = {}
        ...
        created = await call(
            ...
            {
                "folder": "ui-benchmarks/scenario-multi-mcp-20260908-r1",
                "length_mm": 10,
                "block_origins_mm": [[0, 0, 0], [20, 0, 0]],
                "operation_id": "scenario-multi-mcp-create-r2",
            },
        )
        ...
        body_ids = list(dict.fromkeys(f["body"]["id"] for f in faces.structuredContent["faces"]))
        assert len(body_ids) == created.structuredContent["body_count"] == 2
        args = {
            "document": document,
            "path": "ui-benchmarks/scenario-multi-20260908-r1/sources.csv",
            "region_targets": {"SOC": body_ids[0], "SSD": body_ids[1]},
            "format": "csv",
            "metadata": {
            ...
            "operation_id": "scenario-multi-mcp-apply-r2",
        }
        preview = await call(
            "preview",
        ...
        changed = await call(
            ...
            "nx_sim_scenario_apply",
            {
                **args,
                ...
                "operation_id": "scenario-multi-mcp-changed-r2",
            },
        )
        ...
        changed_file = workspace / "ui-benchmarks/scenario-multi-changed-20260908-r1.csv"
        ...
        changed_bytes = await call(
            ...
            "nx_sim_scenario_apply",
            {
                **args,
                ...
                "operation_id": "scenario-multi-mcp-changed-bytes-r2",
            },
        )
        assert (
        ...
        assert applied.structuredContent["applied_internal_heat_W"] == 10
        ...
        conflict = await call(
            "conflict",
            "nx_sim_scenario_apply",
            {**args, "operation_id": "scenario-multi-mcp-conflict-r2"},
        )
        ...
        assert not inventory.isError and inventory.structuredContent["total"] == 2, (
            inventory.structuredContent
        )
        saved = await call(
            "saved",
            "nx_sim_save",
            {"document": document, "operation_id": "scenario-multi-mcp-save-r2"},
        )
