...
def run(executor):
    ...
    expected = executor.workspace.resolve(
        "ui-benchmarks/orthotropic-z-solve-20260908-r1/orthotropic_z_solve_r1.sim"
    )
    ...
    coordinate_system = table.GetCoordinateSystemPropertyValue("material orientation")
    assert coordinate_system is not None
    matrix = coordinate_system.Orientation.Element
    axes = [
        [matrix.Xx, matrix.Xy, matrix.Xz],
        [matrix.Yx, matrix.Yy, matrix.Yz],
        [matrix.Zx, matrix.Zy, matrix.Zz],
    ]
    assert axes == [[0, 1, 0], [0, 0, 1], [1, 0, 0]]
    ...
    shown = executor._sim_show_temperature(
        opened["document"]["id"], name="Reopened third-axis temperature"
    )
