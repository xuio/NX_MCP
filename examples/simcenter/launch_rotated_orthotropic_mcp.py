...
async def main():
    ...
    async with (
        ...
        source = next(
            ...
            for d in docs
            if d["work"]
            and d["path"].endswith("orthotropic-selector-one-20260908-r1\\selector_one_r1.sim")
        )
        responses = {}
        output = Path(
            r"Z:\nx-mcp-integration\simcenter-discovery\stdio-orthotropic-rotated-launch.json"
        )
        ...
        copied = await client.call_tool(
            "nx_sim_save_as",
            {
                ...
                "path": "ui-benchmarks/orthotropic-rotated-solve-20260908-r1/rotated_ortho_solve_r1.sim",
                "operation_id": "orthotropic-rotated-copy-01",
            },
        )
        ...
        prepared = await client.call_tool(
            "nx_sim_prepare_solve", {**args, "operation_id": "orthotropic-rotated-prepare-01"}
        )
        ...
        launched = await client.call_tool(
            "nx_sim_launch", {**args, "operation_id": "orthotropic-rotated-launch-01"}
        )
