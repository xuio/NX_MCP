...
async def main():
    ...
    context = json.loads((shared / "radiation-environment-native.json").read_text())
    env = dict(os.environ)
    env.update(
    ...
    receipt = {"responses": {}}
    output = shared / "radiation-environment-public.json"
    ...
    async with (
        ...
        schema = next(t for t in tools.tools if t.name == "nx_sim_environment_radiation")
        ...
        base = {
            ...
            "provenance": "Assumed effective emissivity; API fixture",
        }
        await call(
            "invalid_dependency",
        ...
        for i, (source, temperature) in enumerate(
            ...
            args = {
                **base,
                ...
                "operation_id": "radiation-environment-" + source + "-r1",
            }
            result = await call(source, "nx_sim_environment_radiation", args)
            replay = await call(source + "_replay", "nx_sim_environment_radiation", args)
            assert result["constraint"]["id"] == replay["constraint"]["id"]
            assert result["temperature_source"] == source
            assert result["solution_membership_verified"] and result["face_count"] == 1
        before = await call(
            "before",
            "nx_sim_constraints",
        ...
        copied = await call(
            ...
            {
                "document": document,
                "path": "ui-benchmarks/R-radiation-environment-export-20260909-r1/radiation_environment_r1.sim",
            },
        )
        ...
        shutil.copy2(exported["input_path"], shared / "radiation-environment.xml")
        receipt.update(
            passed=True,
            document=document,
            path=copied["path"],
