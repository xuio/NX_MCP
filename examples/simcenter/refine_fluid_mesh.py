def run(executor):
    ...
    record = root / "fluid-refinement-01.json"
    if record.exists():
    ...
    backup = root / "before-fluid-refinement-01"
    ...
    if st:
        st.Dispose()
    s.Parts.SetWork(fem)
    mark = s.SetUndoMark(nx.Session.MarkVisibility.Visible, "Refine isolated fluid mesh to 3 mm")
    builder = None
    try:
        ...
        if old_size != 5.0:
            raise ValueError("Expected baseline 5 mm mesh")
        ...
        try:
            ...
            if value != 3.0 or reader.ElementType.ElementTypeName != "Fluid Linear Tetrahedron":
