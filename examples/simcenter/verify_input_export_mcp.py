...
async def main():
    ...
    parser.add_argument("--source-name", default="mcp_saved_copy.sim")
    ...
    async with (
        ...
        copied = await client.call_tool(
            "nx_sim_save_as",
            {
                ...
                "path": f"ui-benchmarks/F-input-export-20260908-r{options.revision}/flow_input_r{options.revision}.sim",
                "operation_id": f"verify-flow-export-copy-20260908-{options.revision:02d}",
            },
        )
        assert not copied.isError, copied.structuredContent
        args = {
            "document": copied.structuredContent["document"]["id"],
            "operation_id": f"verify-flow-export-20260908-{options.revision:02d}",
        }
        exported = await client.call_tool("nx_sim_export_input", args)
        output = Path(r"Z:\nx-mcp-integration\simcenter-discovery\stdio-input-export.json")
        responses = {
        ...
        assert exported.structuredContent["validation"]["mesh_counts"]["elements"] == 186765
        ...
        rejected = await client.call_tool(
            ...
            {
                **args,
                "operation_id": f"verify-flow-export-duplicate-20260908-{options.revision:02d}",
            },
        )
