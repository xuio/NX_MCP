"""Known-good thermal-to-coupled CopyProperty experiment from installed NXOpen.xml; export only."""
...
def run(executor):
    ...
    if "E-environment-expression-20260908-r1" not in sim.FullPath:
        raise ValueError("Activate the retained coupled development SIM")
    sid = executor._reference(sim, "part", sim, "SIM")["id"]
    root = executor.workspace.resolve("ui-benchmarks/E-environment-copy-property-20260908-r1")
    if root.exists():
        raise ValueError("Inspect retained export directory; do not repeat")
    copied = executor._sim_save_as(sid, str(root / "coupled_copy_property_r1.sim"))
    import NXOpen as nx
    ...
    source = next(
        ...
        if p.FullPath.endswith(r"additive-heat-public-20260908-r1\additive_heat_r1.sim")
    )
    source_table = source.Simulation.ActiveSolution.PropertyTable
    ...
    try:
        for key in ("Fluid Temperature", "Absolute Pressure"):
            table.CopyProperty(key, source_table)
            actual_value, actual_unit = table.GetScalarWithDataPropertyValue(key)
            expected_value, expected_unit = source_table.GetScalarWithDataPropertyValue(key)
            assert actual_value == expected_value and actual_unit.Name == expected_unit.Name
    ...
    for path in root.iterdir():
        ...
        if path.is_file() and path.stat().st_size < 8 * 1024 * 1024:
            if path.suffix.lower() == ".xml":
                ...
                shutil.copyfile(
                    path,
                    r"Z:\nx-mcp-integration\simcenter-discovery\coupled-copy-property-deck.xml",
                )
