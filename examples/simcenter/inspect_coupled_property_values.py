...
def run(executor):
    ...
    for kind, count_name, item_name in (
        ...
        try:
            ...
            row['count'] = count
            if not 0 <= count <= 150:
                raise ValueError('Bounded probe requires at most 150 properties')
            row['items'] = []
            for index in range(count):
                tag = getattr(native.Sf, item_name)(solution.Tag, index)
                item = {'index': index, 'tag': int(tag)}
                for name, method in [('name', native.Sf.PropertyAskNameNx), ('property_type', native.Sf.PropertyAskTypeNx), ('value', native.Sf.PropertyAskValueNx)]:
                    try:
                        item[name] = str(method(tag))
