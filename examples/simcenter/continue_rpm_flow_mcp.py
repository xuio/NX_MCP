...
async def main():
    ...
    async with (
        ...
        responses = {}
        output = Path(
            r"Z:\nx-mcp-integration\simcenter-discovery\stdio-rpm-flow-continued-launch.json"
        )
        ...
        activated = await call(
            "activate",
            "nx_sim_activate",
            {"document": source["document"]["id"], "operation_id": "rpm-flow-continue-activate-r1"},
        )
        ...
        assert not tables.isError and tables.structuredContent["total"] == 0
        evidence = json.loads(
            Path(
                r"Z:\nx-mcp-integration\simcenter-discovery\legacy-duct-fan-evidence.json"
            ).read_text()
        )
        assert evidence["status"] == "success" and evidence["path"] == source["path"]
        assert evidence["scale"] == 1.0
        source_curve = {
            "rpm": 1000,
            "pressure_convention": "static",
            "reference_density_kg_m3": 1.2,
            "stall_region": "Synthetic only; no stall model",
            "points": [
                {"flow_m3_s": q, "pressure_Pa": p}
                for q, p in zip(evidence["flow_m3_s"], evidence["pressure_Pa"], strict=True)
            ],
        }
        base = await call(
            "base_curve",
