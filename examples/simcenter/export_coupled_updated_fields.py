...
def run(executor):
    ...
    root = executor.workspace.resolve("ui-benchmarks/E-environment-update-20260908-r1")
    if root.exists():
        raise ValueError("Inspect retained export directory; do not repeat")
    copied = executor._sim_save_as(sid, str(root / "coupled_update_r1.sim"))
    import NXOpen as nx
    ...
    try:
        for key, value, unit in [
            ...
            field = table.GetScalarFieldPropertyValue(key)
            field.EditFieldExpression(str(value), sim.UnitCollection.FindObject(unit), [], True)
            table.SetScalarFieldPropertyValue(key, field)
            assert field.GetFieldExpressionString() == str(value)
    ...
    for path in root.iterdir():
        ...
        if path.is_file() and path.stat().st_size < 8 * 1024 * 1024:
            if path.suffix.lower() == ".xml":
                ...
                shutil.copyfile(
                    path, r"Z:\nx-mcp-integration\simcenter-discovery\coupled-update-deck.xml"
                )
