"""Read the retained active field after native export/reopen; no mutation or solve."""
...
async def main():
    ...
    context = json.loads((shared / "active-field-lifecycle.json").read_text())
    env = dict(os.environ)
    env.update(
    ...
    receipt = {"responses": {}}
    output = shared / "active-field-public-verification.json"
    ...
    async with (
        ...
        tool = next(t for t in discovery.tools if t.name == "nx_sim_loads")
        ...
        compact = await call("compact", "nx_sim_loads", args)
        expanded = await call("expanded", "nx_sim_loads", {**args, "include_properties": True})
        assert compact["loads"] and "properties" not in compact["loads"][0]
        heat = next(p for p in expanded["loads"][0]["properties"] if p["name"] == "Heat Load")
        assert heat == context["after"]["property"]
        assert heat["field_scale"] == 2.0 and heat["evaluated_value"] == 0.2
        assert heat["unit_symbol"] == "W"
        ...
        receipt.update(
            ...
            public_field_matches_native=True,
