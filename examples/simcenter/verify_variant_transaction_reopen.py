...
def run(executor):
    ...
    destination = root / "variant-transaction-20260908-r1"
    expected = {
        destination / name
        for name in ("VariantTxnR1_analysis.sim", "VariantTxnR1_mesh.fem", "VariantTxnR1_cad_01.prt")
    }
    source = root / "D-independent-refined-solve-20260908-r1/refined_flow_r1.sim"
    base = root / "D-independent-clone-native-20260908-r1"
    originals = [
        source,
        base / "independent_mesh.fem",
        base / "independent_geometry.prt",
    ]
    ...
    try:
        ...
        elements.Dispose()
        nodes.Dispose()
    assert counts == {"elements": 261589, "nodes": 98632}, counts
