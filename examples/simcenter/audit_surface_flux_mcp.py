...
async def main():
    ...
    async with (
        ...
        output = Path(r"Z:\nx-mcp-integration\simcenter-discovery\flux-solve-finish.json")
        ...
        while True:
            status = await client.call_tool(
                "nx_sim_job_status",
                {"job_id": "surface-flux-numerical-r1", "include_evidence": True},
            )
        ...
        release = await client.call_tool(
            "nx_sim_release_job",
            {"job_id": "surface-flux-numerical-r1", "operation_id": "flux-release-r1"},
        )
        ...
        document = next(
            ...
            if d["work"] and d["path"].endswith("flux_solve_r1.sim")
        )
        ...
        identity = await client.call_tool(
            "nx_sim_result_identity", {"document": document, "job_id": "surface-flux-numerical-r1"}
        )
        ...
        shown = await client.call_tool(
            ...
            {"document": document, "operation_id": "flux-show-r1"},
        )
        responses["visible_result"] = shown.model_dump(mode="json")
        record()
        assert not shown.isError, shown.structuredContent
        print(
            "Surface-flux solver exited; temperature result retrieved. Heat-balance acceptance is separate."
        )
