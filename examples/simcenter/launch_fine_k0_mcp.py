...
async def main():
    ...
    async with (
        ...
        source = next(
            d
            for d in docs
            if d["work"] and d["path"].endswith("D-fine-k0-20260908-r1\\fine_k0_r1.sim")
        )
        responses = {}
        output = Path(r"Z:\nx-mcp-integration\simcenter-discovery\stdio-fine-k0-launch.json")
        ...
        args = {
            ...
            "job_id": "fine-k0-flow-01",
        }
        prepared = await client.call_tool(
            "nx_sim_prepare_solve", {**args, "operation_id": "fine-k0-prepare-01"}
        )
        ...
        launched = await client.call_tool(
            "nx_sim_launch", {**args, "operation_id": "fine-k0-launch-01"}
        )
