...
async def main():
    ...
    async with (
        ...
        output = Path(r"Z:\nx-mcp-integration\simcenter-discovery\stdio-live-revision-finish.json")
        ...
        while True:
            status = await client.call_tool(
                "nx_sim_job_status",
                {"job_id": "live-revision-thermal-01", "include_evidence": True},
            )
        ...
        release = await client.call_tool(
            "nx_sim_release_job",
            {"job_id": "live-revision-thermal-01", "operation_id": "live-revision-release-01"},
        )
        ...
        document = next(
            ...
            if d["work"] and d["path"].endswith("live_revision_solve_r1.sim")
        )
        ...
        identity = await client.call_tool(
            "nx_sim_result_identity", {"document": document, "job_id": "live-revision-thermal-01"}
        )
        ...
        shown = await client.call_tool(
            ...
            {"document": document, "operation_id": "live-revision-show-01"},
        )
