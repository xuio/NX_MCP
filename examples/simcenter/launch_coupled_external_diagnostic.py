...
def run(executor):
    ...
    if "E-external-public-20260908-r1" not in sim.FullPath:
        raise ValueError("Unexpected current diagnostic source")
    receipt = executor.workspace.resolve("ui-benchmarks/E-external-diagnostic-r1-launch.json")
    if receipt.exists():
        return {
            "replayed": True,
    ...
    record(
        ...
        {
            "acceptance": False,
            "general_ambient_export_limitation": True,
    ...
    record(
        ...
        executor._sim_save_as(
            sid,
            "ui-benchmarks/E-external-diagnostic-20260908-r1/coupled_external_diagnostic_r1.sim",
        ),
    )
    ...
    try:
        ...
        assert value == 0 and unit.Name == "Celsius"
        assert solution.PropertyTable.GetIntegerPropertyValue("Ambient Pressure") == 1
        record(
            "declared_environment",
            {
                "global_reference_temperature_C": 0,
                "external_boundary_temperature_C": 20,
                "ambient_pressure_mode": "from_altitude",
                "acceptance": False,
                "limitation": "General ambient export remains unresolved; test explicit external temperatures only",
            },
        )
        controls = []
        for key, property_name in [
            ...
            table.SetIntegerPropertyValue(property_name, 500)
    ...
    record("prepare", executor._sim_prepare_solve(sid, "coupled-external-diagnostic-r1"))
    record("prepare_seconds", time.monotonic() - started)
    record(
        "launch_intent",
        "Inspect persistent job coupled-external-diagnostic-r1 after any interruption; never relaunch",
    )
    record("launch", executor._sim_launch(sid, "coupled-external-diagnostic-r1"))
    return rows
