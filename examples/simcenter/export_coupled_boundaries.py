...
def run(executor):
    ...
    root = executor.workspace.resolve("ui-benchmarks/E-boundaries-export-20260908-r1")
    if root.exists():
        raise ValueError("Inspect retained export directory; do not repeat")
    copied = executor._sim_save_as(sid, str(root / "coupled_boundaries_r1.sim"))
    import NXOpen as nx
    import NXOpen.UF as uf
    step = sim.Simulation.ActiveSolution.ActiveStep
    old = step.PropertyTable.GetIntegerPropertyValue("Solution Type")
    mark = executor.session.SetUndoMark(nx.Session.MarkVisibility.Visible, "MCP coupled steady step")
    try:
    ...
    for path in root.iterdir():
        ...
        if path.is_file() and path.stat().st_size < 8 * 1024 * 1024:
            if path.suffix.lower() == ".xml":
                ...
                import shutil
                shutil.copyfile(path, r"Z:\nx-mcp-integration\simcenter-discovery\coupled-boundaries-deck.xml")
            if path.suffix.lower() == ".log":
                row["log"] = inspect_solver_log(path.read_text(errors="replace"))
        files.append(row)
    return {
        "copy": copied,
        "step_type": {"before": old, "after": actual, "interpretation": "steady hypothesis pending native export validation"},
        "setup_report": report,
        "native_error": error,
        "files": files,
