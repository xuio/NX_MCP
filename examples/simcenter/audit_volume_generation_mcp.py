...
async def main():
    ...
    async with (
        ...
        output = Path(r"Z:\nx-mcp-integration\simcenter-discovery\volume-solve-finish.json")
        ...
        while True:
            status = await client.call_tool(
                "nx_sim_job_status",
                {"job_id": "volume-generation-numerical-r1", "include_evidence": True},
            )
        ...
        release = await client.call_tool(
            "nx_sim_release_job",
            {"job_id": "volume-generation-numerical-r1", "operation_id": "volume-release-r1"},
        )
        ...
        identity = await client.call_tool(
            "nx_sim_result_identity", {"document": document, "job_id": "volume-generation-numerical-r1"}
        )
        ...
        shown = await client.call_tool(
            ...
            {"document": document, "operation_id": "volume-show-r1"},
        )
        responses["visible_result"] = shown.model_dump(mode="json")
        record()
        assert not shown.isError, shown.structuredContent
        print(
            "Volume-generation solver exited; temperature result retrieved. Heat-balance acceptance is separate."
        )
