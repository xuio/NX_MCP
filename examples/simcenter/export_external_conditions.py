"""Documented solve-manager export experiment from installed NXOpen.xml; export only."""


def run(executor):
    import time

    import NXOpen.CAE as cae

    from nx_mcp.simcenter.solver_guard import require_solver_idle
    from nx_mcp.simcenter.solver_log import inspect_input_xml, inspect_solver_log

    require_solver_idle()
    executor._sim_open(
        str(
            executor.workspace.resolve(
                "ui-benchmarks/E-documented-controls-20260908-r1/coupled_documented_controls_r1.sim"
            )
        )
    )
    sim = executor.session.Parts.BaseWork
    if "E-documented-controls-20260908-r1" not in sim.FullPath:
        raise ValueError("Activate the retained coupled development SIM")
    sid = executor._reference(sim, "part", sim, "SIM")["id"]
    root = executor.workspace.resolve("ui-benchmarks/E-external-conditions-20260908-r1")
    if root.exists():
        raise ValueError("Inspect retained export directory; do not repeat")
    copied = executor._sim_save_as(sid, str(root / "coupled_external_conditions_r1.sim"))
    import NXOpen as nx
    import NXOpen.UF as uf

    from nx_mcp.simcenter.properties import read_properties

    mark = executor.session.SetUndoMark(
        nx.Session.MarkVisibility.Visible, "MCP documented external conditions"
    )
    try:
        conditions = sim.ModelingObjectPropertyTables.CreateModelingObjectPropertyTable(
            "External Conditions",
            "NX MULTIPHYSICS - Coupled Thermal-Flow",
            "NX MULTIPHYSICS",
            "External air 20 C",
            0,
        )
        props = conditions.PropertyTable
        props.SetIntegerPropertyValue("Temperature Option", 0)
        expression = sim.Expressions.CreateSystemNumberExpression(
            "20", sim.UnitCollection.FindObject("Celsius")
        )
        wrapper = sim.FieldManager.CreateScalarFieldWrapperWithExpression(expression)
        props.SetScalarFieldWrapperPropertyValue("Temperature Value", wrapper)
        assigned = []
        for obj in sim.Simulation.SimulationObjects:
            if obj.Name in ("Coupled Inlet", "Coupled Opening"):
                assert (
                    obj.PropertyTable.GetNamedPropertyTablePropertyValue("Inlet Conditions") is None
                )
                obj.PropertyTable.SetNamedPropertyTablePropertyValue("Inlet Conditions", conditions)
                assert (
                    obj.PropertyTable.GetNamedPropertyTablePropertyValue("Inlet Conditions")
                    == conditions
                )
                assigned.append(obj.Name)
        assert len(assigned) == 2
        readback = read_properties(props, nx)
    except Exception:
        executor.session.UndoToMark(mark, None)
        raise
    status = sim.Save(nx.BasePart.SaveComponents.FalseValue, nx.BasePart.CloseAfterSave.FalseValue)
    if status:
        status.Dispose()
    started = time.monotonic()
    error = None
    try:
        manager = cae.SimSolveManager.GetSimSolveManager(executor.session)
        prerequisites = manager.GetChainOfPrerequisites(sim.Simulation.ActiveSolution)
        counts = manager.SolveChainOfSolutions(
            [sim.Simulation.ActiveSolution],
            cae.SimSolutionSolveOption.WriteSolverInputFile,
            cae.SimSolutionSetupCheckOption.CompleteCheckAndOutputErrors,
            cae.SimSolutionSolveMode.Foreground,
        )
    except Exception as exc:
        error = {
            "type": type(exc).__name__,
            "nx_code": getattr(exc, "ErrorCode", None),
            "message": str(exc),
        }
    report_path = root / "setup-report.txt"
    uf.UFSession.GetUFSession().Ui.SaveListingWindow(str(report_path))
    report = report_path.read_text(errors="replace")
    report = report[report.rfind("          Solution Model Setup Check Error Summary") :]
    files = []
    for path in root.iterdir():
        row = {"name": path.name, "bytes": path.stat().st_size}
        if path.is_file() and path.stat().st_size < 8 * 1024 * 1024:
            if path.suffix.lower() == ".xml":
                row["input"] = inspect_input_xml(path.read_bytes())
                import shutil

                shutil.copyfile(
                    path,
                    r"Z:\nx-mcp-integration\simcenter-discovery\coupled-external-conditions-deck.xml",
                )
            if path.suffix.lower() == ".log":
                row["log"] = inspect_solver_log(path.read_text(errors="replace"))
        files.append(row)
    return {
        "copy": copied,
        "external_conditions": readback,
        "assigned_boundaries": assigned,
        "manager_counts": list(counts) if error is None else None,
        "prerequisites": [[s.Name for s in prerequisites[0]], list(prerequisites[1])]
        if error is None
        else None,
        "setup_report": report,
        "native_error": error,
        "files": files,
        "elapsed_seconds": time.monotonic() - started,
        "solver_launched": False,
        "numerical_acceptance": False,
        "boundary_conditions_complete": False,
    }
