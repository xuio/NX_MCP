...
def run(executor):
    ...
    s = executor.session
    sim = next(p for p in s.Parts if p.FullPath.endswith("contact_context_r2.sim"))
    ...
    try:
        ...
        face = next(row["face"] for row in inventory["rows"] if int(row["face"].Tag) not in used)
