...
async def main():
    ...
    async with (
        ...
        sim_path, fem_path = folder / "public_open_r1.sim", folder / "public_open_fem_r1.fem"
        if sim_path.exists() or fem_path.exists():
            raise ValueError("Fresh native open fixture paths required")
        shutil.copy2(folder / "reopen_modified_probe_r1.sim", sim_path)
        shutil.copy2(next(folder.glob("*_mesh.fem")), fem_path)
        output = stage / "stdio-sim-open.json"
        responses = {}
        ...
        first = await call(
            "fresh_sim", {"path": str(sim_path), "operation_id": "sim-open-fresh-r1"}
        )
        assert not first.isError and not first.structuredContent["already_loaded"], (
            first.structuredContent
        )
        reused = await call(
            "reuse_sim",
            {"path": str(sim_path.relative_to(workspace)), "operation_id": "sim-open-reuse-r1"},
        )
        ...
        fem = await call("fresh_fem", {"path": str(fem_path), "operation_id": "fem-open-fresh-r1"})
        assert not fem.isError and not fem.structuredContent["already_loaded"], (
            fem.structuredContent
        )
        back = await call(
            "return_sim", {"path": str(sim_path), "operation_id": "sim-open-return-r1"}
        )
        assert not back.isError and back.structuredContent["already_loaded"], back.structuredContent
        missing = await call(
            "missing", {"path": str(folder / "missing.sim"), "operation_id": "sim-open-missing-r1"}
        )
