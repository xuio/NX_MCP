...
async def main():
    ...
    async with (
        ...
        conflict = await call(
            "conflict", coefficient=2, expected_coefficient=3, operation_id="head-loss-conflict-r1"
        )
        assert conflict.isError
        changed = await call(
            "changed", coefficient=2, expected_coefficient=0, operation_id="head-loss-set-r1"
        )
        assert not changed.isError and changed.structuredContent["coefficient"] == 2
        replay = await call(
            "replay", coefficient=2, expected_coefficient=0, operation_id="head-loss-set-r1"
        )
        assert not replay.isError and replay.structuredContent["coefficient"] == 2
        unchanged = await call(
            "unchanged", coefficient=2, expected_coefficient=2, operation_id="head-loss-noop-r1"
        )
        assert not unchanged.isError and unchanged.structuredContent["action"] == "unchanged"
        restored = await call(
            "restored", coefficient=0, expected_coefficient=2, operation_id="head-loss-restore-r1"
        )
