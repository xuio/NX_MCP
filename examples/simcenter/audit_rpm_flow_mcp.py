...
async def main():
    ...
    async with (
        ...
        output = Path(r"Z:\nx-mcp-integration\simcenter-discovery\stdio-rpm-flow-finish.json")
        ...
        while True:
            status = await client.call_tool(
                "nx_sim_job_status", {"job_id": "rpm-flow-1100-r1", "include_evidence": True}
            )
        ...
        release = await client.call_tool(
            "nx_sim_release_job",
            {"job_id": "rpm-flow-1100-r1", "operation_id": "rpm-flow-release-r1"},
        )
        ...
        audit = await client.call_tool(
            "nx_sim_flow_log",
            {
                "job_id": "rpm-flow-1100-r1",
                "log_name": "rpm_flow_1100_r1-Flow_benchmark.log",
                "limit": 1,
            },
        )
        ...
        document = next(
            ...
            if d["work"] and d["display"] and d["path"].endswith("rpm_flow_1100_r1.sim")
        )
        shown = await client.call_tool(
            "nx_sim_show_pressure",
            {
                ...
                "operation_id": "rpm-flow-show-pressure-r1",
            },
        )
