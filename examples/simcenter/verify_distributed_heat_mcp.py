...
async def main():
    ...
    async with (
        ...
        output = Path(r"Z:\nx-mcp-integration\simcenter-discovery\stdio-distributed-heat.json")
        responses = {}
        ...
        await call(
            ...
            {
                "folder": "ui-benchmarks/distributed-heat-public-20260908-r1",
                "length_mm": 10,
                "block_origins_mm": [[0, 0, 0], [20, 0, 0]],
                "operation_id": "distributed-heat-create-r1",
            },
        )
        ...
        await call(
            ...
            "nx_sim_distributed_heat",
            {
                **common,
                ...
                "operation_id": "distributed-heat-wrong-kind-r1",
            },
            error=True,
        )
        args = {
            **common,
            ...
            "operation_id": "distributed-heat-flux-r1",
        }
        ...
        await call(
            "duplicate",
            "nx_sim_distributed_heat",
            {**args, "name": "MCP duplicate", "operation_id": "distributed-heat-duplicate-r1"},
            error=True,
        )
        volume = await call(
            "volume",
            "nx_sim_distributed_heat",
            {
                **common,
                ...
                "operation_id": "distributed-heat-volume-r1",
            },
        )
        assert volume["value"] == 100000 and volume["units"] == "W/m^3"
        await call(
            "save", "nx_sim_save", {"document": doc, "operation_id": "distributed-heat-save-r1"}
        )
        await call(
            "close", "nx_sim_close", {"document": doc, "operation_id": "distributed-heat-close-r1"}
        )
        await call(
            "open", "nx_sim_open", {"path": path, "operation_id": "distributed-heat-open-r1"}
        )
