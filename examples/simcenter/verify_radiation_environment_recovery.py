...
def run(executor):
    ...
    from nx_mcp.simcenter import radiation
    ...
    assert sim.FullPath.endswith("radiation_environment_r1.sim")
    flags = {p.FullPath: bool(p.IsModified) for p in session.Parts}
    shared = Path(r"Z:\nx-mcp-integration\simcenter-discovery")
    initial = json.loads((shared / "radiation-environment-native.json").read_text())
