"""Bounded CAE scalar setter experiment from installed NXOpen.xml; export only."""
...
def run(executor):
    ...
    if "E-diagnostic-20260908-r1" not in sim.FullPath:
        raise ValueError("Activate the retained coupled development SIM")
    sid = executor._reference(sim, "part", sim, "SIM")["id"]
    root = executor.workspace.resolve("ui-benchmarks/E-environment-cae-scalar-20260908-r1")
    if root.exists():
        raise ValueError("Inspect retained export directory; do not repeat")
    copied = executor._sim_save_as(sid, str(root / "coupled_cae_scalar_r1.sim"))
    import NXOpen as nx
    ...
    for path in root.iterdir():
        ...
        if path.is_file() and path.stat().st_size < 8 * 1024 * 1024:
            if path.suffix.lower() == ".xml":
                ...
                shutil.copyfile(
                    path, r"Z:\nx-mcp-integration\simcenter-discovery\coupled-cae-scalar-deck.xml"
                )
