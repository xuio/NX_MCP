...
class SimcenterMixin:
    ...
    def _sim_internal_fan_flow(self, document, boundary, volume_flow_m3_s):
        import NXOpen.CAE as cae

        from nx_mcp.simcenter.internal_fan_edit import edit
    ...
    def _sim_internal_fan_curve(self, document, boundary, fan_table):
        import NXOpen.CAE as cae

        from nx_mcp.simcenter.internal_fan_curve import assign
        from nx_mcp.simcenter.solver_guard import require_solver_idle
        ...
        if not isinstance(sim, cae.SimPart):
            raise NXToolError("NX_SIM_DOCUMENT_TYPE", "Select a SIM")
        fan = self.objects.resolve(boundary, expected_kind="simulation_object")
        table = self.objects.resolve(fan_table, expected_kind="simulation_field")
        result = assign(self.session, sim, fan, table)
        return {"document": self._reference(sim, "part", sim, "SIM"), **result}
    ...
    def _sim_create_analysis(self, cad_document, folder, name, analysis_type="coupled_thermal_flow", bodies=None):
        from nx_mcp.simcenter.analysis_documents import create

        cad = self.objects.resolve(cad_document, expected_kind="part")
        return create(self, cad, folder, name, analysis_type, bodies)
    ...
    def _sim_mesh_plan(self, document, regions, number_of_processors=None, surface_meshing_method=None):
        from nx_mcp.simcenter.mesh_plan import generate

        fem = self.objects.resolve(document, expected_kind="part")
        try:
            return generate(
                self, fem, regions, number_of_processors=number_of_processors,
                surface_meshing_method=surface_meshing_method,
            )
        except ValueError as error:
            raise NXToolError("NX_INVALID_ARGUMENT", str(error)) from error
    ...
    def _sim_mass_flux_samples(
        self, document, result_sha256, node_labels,
        loadcase_index=0, iteration_index=0, maximum_bytes=1_073_741_824,
    ):
        ...
        return read_bound(
            self, document, result_sha256, mass_flux_samples, maximum_bytes,
            node_labels=node_labels, loadcase_index=loadcase_index,
            iteration_index=iteration_index,
        )
    ...
    def _sim_flow_convergence(
        ...
        result = configure_convergence(
            ...
            heat_imbalance_fraction=heat_imbalance_fraction,
        )
        return {"document": self._reference(sim, "part", sim, "part"), **result}
