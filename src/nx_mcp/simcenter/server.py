...
def nx_sim_result_identity(
    ...
    """Inspect associated result-file paths, sizes, SHA256 hashes and NX verification for an active SIM document ID. maximum_bytes is a positive total hashing budget (default 1 GiB); files outside the workspace are rejected. Does not load fields, solve, save or alter result associations. File identity and NX verification do not establish current-model freshness: result_freshness is not_verified unless a supported live-state comparison proves it stale; engineering_accepted remains false. Preserve a pre-solve dependency manifest and job identity for that separate audit. Missing files and files changing during inspection are errors. Optional job_id binds to an existing job in simcenter-jobs: rejects a different analysis/solution, compares the associated result with the observed job artifact, and audits supplied pre-solve dependency hashes and current modification flags. When the job records a labelled mesh baseline, compare current mesh using its recorded inspection budget. Missing legacy baselines and failed mesh inspection remain not_verified; changed mesh makes result_freshness stale. Matching mesh is not whole-model freshness. maximum_bytes covers results plus dependencies. A SIM hash mismatch can reflect solve metadata; it does not establish a physics change. No audit result certifies numerical validity or complete model freshness."""
...
def nx_sim_heat_power(
    ...
    """Assign total internal heat in finite nonnegative watts to one prototype FEM body ID from nx_sim_faces. Requires an active NX MULTIPHYSICS Thermal or Coupled Thermal-Flow SIM with one direct FEM occurrence. Returns a typed simulation_load reference and committed SIM body/power readback. Rejects an existing heat load on that body and duplicate names. Nonempty provenance is stored on the native load. Only internal heat belongs here: do not add exported electrical energy or battery storage, or sum converter input and output as heat. Visible undo/rollback; does not save or solve. Time-dependent and multi-body power assignment are not exposed here. Supply operation_id for safe retry. Face/body ownership overlaps are rejected by default; allow_additive explicitly permits separate intended contributions. Exact-target duplicate total-power loads remain rejected; distinct-body intersections are not checked."""
...
def nx_sim_mesh_plan(
    document: str, regions: list[BodyMeshRegion], number_of_processors: int | None = None,
    surface_meshing_method: Literal["standard", "mesh_from_facets"] | None = None,
):
    """Generate a complete explicit body mesh plan in an active millimeter FEM with no existing meshes. Supply a nonempty list of regions, each exactly {body: typed FEM body ID, kind: 'solid'|'fluid', size_mm: finite (0,10000]}. Cover every FEM body once. Optional number_of_processors is an integer 1..32, applied and verified on every builder and committed mesh; omitted preserves native defaults. Optional surface_meshing_method selects standard or mesh_from_facets, verified before and after commit; omission preserves the native default and does not alter cleanup tolerances. Writes per-region start/commit markers to the native session log for crash diagnosis. These settings do not guarantee serial execution of every NX subsystem. Uses installed Linear Tetrahedron or Fluid Linear Tetrahedron; respects native mesh controls and can produce multiple meshes per body. Verifies primary mesh element type, size and body selection by reopening its builder; returns meshes per region and total element/node counts. Does not infer fluid regions, assign materials, save or solve. Control effectiveness, element quality and convergence require separate inspection. Rejects foreign/stale/duplicate/missing bodies and running solvers. Entire plan uses undo on failure; partial recovery is explicit. Supply operation_id for safe retry; never rerun because a transport response timed out."""
...
def nx_sim_gravity(document: str, bodies: list[str], acceleration_m_s2: list[float], name: str):
    """Create whole-FEM constant gravity on explicit FEM prototype body IDs from nx_sim_faces; the list must cover every body in the active coupled thermal-flow SIM. Supply three finite XYZ components in m/s², global Cartesian frame, and a unique name. Requires one direct FEM occurrence. Returns committed SI vector, typed load and target references, and solution membership. Duplicate names and overlapping supported gravity loads are rejected. Zero vector is allowed. Does not enable buoyancy: configure that separately with nx_sim_environment. No save, solve or material-law change. Uses creation rollback and operation_id deduplication. Selective-body gravity, rotating frames, time-varying gravity and nested FEM assemblies are unsupported. Native CSYS 0 exports as XML selector 1; this representation difference remains documented, not interpreted as a solver correction."""
...
def nx_sim_internal_fan_flow(document: str, boundary: str, volume_flow_m3_s: float):
    """Edit one existing Internal Fan's positive constant volume flow in m3/s. Requires idle known solver processes, active single-step Multiphysics Flow/Coupled SIM, same-owner active-solution fan, constant m3/s expression, normal alignment and no swirl/controller. Creates a new expression/wrapper to avoid modifying a shared expression. Verifies direction, motor heat, target set 0, selectors and object/solution membership. Does not rename the fan, save, export, solve or qualify its operating point. Visible undo with binding/inventory rollback checks. Native acceptance pending; audit exact exported delta on an isolated study before solving."""
...
def nx_sim_mesh_mating(document: str, source_face: str, target_face: str, tolerance_mm: float = 0.001, allow_contained: bool = False, area_relative_tolerance: float = 1e-6):
    """Create one native manual Glue Coincident mesh-mating condition in an active standalone millimeter FEM with no generated meshes. Existing readable mesh-mating conditions are preserved; add sizing controls later. Duplicate face pairs are rejected. Select two distinct FEM-prototype faces on different bodies with bounding boxes agreeing within tolerance_mm (finite (0,0.1] mm); bounds are not exact-coincidence proof. Optional allow_contained requires a smaller axis-aligned face within the larger face bounds on the same plane, including shared outer edges; it uses native AllPairs search and requires one shared face preserving the complete smaller native area. Contained area_relative_tolerance defaults to 1e-6 and must be finite in (0,2e-4]; only change it with measured native-area evidence and an independent mesh-area acceptance check. It affects the area comparison only, retains the 1e-6 mm^2 absolute floor, and does not relax face bounds, shared-face membership or distance/snap checks. Reports expected/actual areas and tolerances, including on rollback. Native circular imprinting has shown polygon-area readback changes up to 152 ppm; the default remains 1e-6, and larger explicit allowances do not certify the geometry or mesh; generated mesh area/connectivity still need validation. This experimental mode needs native validation. Sets distance/snap tolerances, verifies committed selections/mode, and attempts rollback on failure. Invalidates FEM and dependent SIM references after a native attempt; reacquire documents/faces. Does not mesh, save, merge arbitrary nodes or prove solver connectivity. Inspect the generated mesh and native solver before accepting an internal fan interface. Native authoring of two sequential interfaces and preservation of the first condition were validated on three touching blocks in NX2606. Partial-face contacts and generated multi-interface mesh connectivity remain unverified. Use operation_id to avoid replay after timeouts."""
